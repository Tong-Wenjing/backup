...
function genStatTable() {
        ...
        for (var i = 0; i < app_sum.table_tr_arr.length; i++)
        {
            ...
            for (tmp_title in app_sum.table_tr_data)
            {
                if (tmp_title == title)
                {
                	var tmp_object = format(app_sum.table_tr_data[title]);
                    var iter = true;
                    for (sub_title in tmp_object)
                    {
                        sub_value = tmp_object[sub_title];
                        if (iter)
                        {
...
/*
*	Since the object generated by Python doesn't have order, it's
*	needed to sort the object follow the order of key
*	Params:
*		object: 	Python object
*	Return:
*		object: 	Javascript object (which has been sorted on the keys)
*/
function format(object){
	...
	//Transfer the Python object key to temp Array
	for (object_key in object)
	{
		tmp_array[i] = object_key;	
		i++;
	}

	//Sort the temp Array by default
	tmp_array.sort();	

	//build a Javascript object with the formatted key and original value
	for (var j = 0; j < tmp_array.length; j++)
	{
		object_value = object[tmp_array[j]];
		tmp_array[j] = tmp_array[j].slice(tmp_array[j].indexOf('-') + 1);
		tmp_object[tmp_array[j]] = object_value;
	}
